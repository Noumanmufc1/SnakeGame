...
framePerSecond = pygame.time.Clock() #runs a clock while the game is being executed; it enables to define frames per second for the game.

screenDisplay = pygame.display.set_mode((displayWidth,displayHeight))

pygame.display.set_caption("Snake") #Sets the title of the window that pops up when you open the game.


pixelSize = 10#the size of the red boxes and the increase in the length snake per box
movingDirection = "east" # initial moving direstion of snake
...
def snakeMaking(pixelSize, makeSnakeList):					#function for snake making
...
# rotating the head of the image according to the direction
	...
	screenDisplay.blit(head1, (makeSnakeList[len(makeSnakeList)-1][0], makeSnakeList[len(makeSnakeList)-1][1]))	#bliting the sanke on to the screen
	
	for [X,Y] in makeSnakeList[:-1]:
		pygame.draw.rect(screenDisplay,blue,[X,Y,pixelSize,pixelSize]) #for each box in the snake list, drawing a rectangle to make the snake



def messageOnScreen(message, color, y = 0, capacity= 0): # defining the function for displaying a message on the screen, y determines the displacement of the message form the center, and capacity determines the font
	...
	rectText = surfaceOfText.get_rect()#creating a rectangle of the above font
	

	rectText.center = (displayWidth/2, displayHeight/2-y)#centering the rectangle created above

	screenDisplay.blit(surfaceOfText,rectText)#bliting the text on to the rectangle which appears on the screen


def types(): # creating a finction for the different types in the game
	types = True
	global mode
	while types:
		...
		for event in pygame.event.get():						
			if event.type == pygame.QUIT:				# quits the game if the users quits it
				pygame.quit()
				quit()
			if(event.type == pygame.KEYDOWN):				#checks for the pressing of the key
				if(event.key == pygame.K_c): 				# if c is pressed the mode is classic
					mode = "classic"
					types = False
				elif(event.key == pygame.K_m):					# if m is pressed the mode is modern/cave
					mode = "modern"
					types = False
...
def levels():
	...
	while level:
		...
		messageOnScreen("Press 1 for Level 1:", red, 40, 1)
		messageOnScreen("Press 2 for Level 2:", red, 20, 1)
		messageOnScreen("Press 3 for Level 3:", red, 0, 1)
		messageOnScreen("Press 4 for Level 4:", red, -20, 1)
		messageOnScreen("Press 5 for Level 5:", red, -40, 1)
...
def startScreen():					#for the start screen display
...
def mainGame():
	...
	#bgSound.play(1000)
	global movingDirection				#using global variable movingDirection in the function
	...
	while runningGame:
		while gameOver==True:
			...
			messageOnScreen("Your score was: " + str(maxSnakeLength), black, 0, 2)
			...
			for event in pygame.event.get():								#running the loop for every event in the game
				if (event.type == pygame.QUIT ):							#checking if the event calls for quiting the game, if it does then quits the loop by changing the variables to False
					runningGame = False
					gameOver = False
				if(event.type == pygame.KEYDOWN):							#checks for the pressing of a key
		...
		X += changeX	#the change in the coordinates is added in the respective X or Y
		Y += changeY


		screenDisplay.fill(white)
		pygame.draw.rect(screenDisplay, red, [appleX, appleY, pixelSize, pixelSize])# creating the red box
		...
		if len(makeSnakeList) > maxSnakeLength:
			del makeSnakeList[0]

		for eachBlock in makeSnakeList[:len(makeSnakeList)-1]: # checking for each block in the list of snake
			if(eachBlock == headOfSnake):					# if the snake cuts itself at any point the game is over
				gameOver = True

		pygame.display.update()
		if X == appleX and Y == appleY:						# if the snake's cordinates conincides with the red box's then other random X and Y coordinates are generated for a new red box
			#eatApple.play(1)
			appleX = round(random.randrange(0, displayWidth - pixelSize)/10.0)*10.0
			appleY = round(random.randrange(0, displayHeight - pixelSize)/10.0)*10.0
			maxSnakeLength += 1				#adding one to the length each time the snake eats a red box
		score = font1.render("Score: " +str(maxSnakeLength), True, blue)					# writing score
		screenDisplay.blit(score, [0,0])								#bliting the score on to the top left corner of the screen
		pygame.display.update()


		framePerSecond.tick(FPS)					#moving the screen according to the frames given per second


	pygame.quit()	#quits the game if comes out of the loop
	quit()
